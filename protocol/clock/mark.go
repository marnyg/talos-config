...
// every wallet-rooted cert carries iat, and a verifier keeps
// lw = max(lw, iat) over every cert whose SIGNATURE it verified, then
// judges with now = max(local, lw). The advance is uncapped: capping it
// at local+skew discards exactly the honest evidence a rollback needs
// (clock.qnt FINDING).
//
...
// Observe folds a cert's iat into the mark. Call it ONLY after the
// cert's signature has been verified (cert.Verify / a truthy
// cert.Authorize Verified entry): an unverified iat is attacker-chosen.
// The advance is monotone and uncapped.
...
// verified in the chain (the "update first" step of ADR-0019).
