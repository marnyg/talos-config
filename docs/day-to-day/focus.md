...
envelopes from the outside world through its own relay. **Now: the
first real member.** `359.8.3` (cp1 agent: NodeId on iroh, enroll,
beat, `policy.AcceptTable(KindNode)`) and `359.8.4` (irohup) consume
the Kit; the name map half of `#bundle` (`359.8.2.3`) fills from their
piggybacked locations; `kql` tears the scratch relay down; exit checks
are event-based (`359.8.6`).
