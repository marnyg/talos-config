...
- 2026-07-29 — ~~`nix run .#apply` is dangerous on provisioned machines~~ <!-- stale? -->
  Fixed later same day (`4dcfb659`): apply now fetches the hub-composed config
  over the tunnel and refuses to compose locally.
- 2026-07-29 — Every fly deploy re-seals the hub: derived roles (wg, <!-- stale? -->
...
- 2026-07-29 — ~~`nix run .#apply` now requires being on the wg tunnel as <!-- stale? -->
  ...
  `http://10.42.0.1`.
- 2026-07-29 — ~~`wgping` binaries built before the tunnel-HTTP change <!-- stale? -->
  hang against the new hub.~~ Tool deleted with wg0 (2026-07-30);
  offline derivations moved to `cmd/recover`.
- 2026-07-29 — nebula cert-version skew: `nebula-cert` ≥1.10 emits V2 <!-- stale? -->
...
- 2026-07-29 — mesh names resolve but nothing answers on them yet: <!-- stale? -->
...
- 2026-07-29 — the node's mesh identity arrives with its *config*, and <!-- stale? -->
...
- 2026-07-29 — a node's overlay firewall lives in its *stored config*, so <!-- stale? -->
...
- 2026-07-29 — the laptop's wireguard interface is named `talos-laptop`, <!-- stale? -->
...
- 2026-07-29 — **any mesh measurement taken with the wg tunnel up is <!-- stale? -->
...
- 2026-07-29 — first mesh tunnel takes ~6s to converge: the initial <!-- stale? -->
...
- 2026-07-29 — the upgrade→apply gap is safe to sit in: cp1 spent ~5 <!-- stale? -->
...
- 2026-07-29 — a mesh derivation error (address collision, bad `meshIP`) <!-- stale? -->
...
- 2026-07-29 — ~~the mesh is enabled per-deploy with `--mesh-port` and <!-- stale? -->
...
- 2026-07-30 — **Any overlay carrying the route to the hub/peer poisons a <!-- stale? -->
...
- 2026-07-30 — **The office MacBook is enrolled as device name `laptop`**, <!-- stale? -->
...
- 2026-07-30 — Home network has **no native IPv6** (no v6 default route; <!-- stale? -->
...
- 2026-07-30 — **cp1's LAN lease drifts freely**: 10.0.0.20→.30 <!-- stale? -->
...
- 2026-07-30 — **Mobile Nebula on the phone was configured by hand**: <!-- stale? -->
...
- 2026-07-31 — **the cluster endpoint is now cp1's mesh address** <!-- stale? -->
...
- 2026-07-31 — anything cached that points at `10.99.0.54` (old <!-- stale? -->
  ...
  at `10.42.218.125`.
- 2026-07-30 — **the hub re-mints its own nebula leaf at every unseal**: <!-- stale? -->
...
- 2026-07-30 — **wg0 is fully deleted** (phase 2 step 3): anything <!-- stale? -->
...
- 2026-07-30 — **first `apply` after a hub redeploy can time out**: <!-- stale? -->
...
- 2026-07-30 — ~~the cached `~/.config/talos-mesh/laptop.yml` predates <!-- stale? -->
...
- 2026-07-30 — hub-redeploy mesh warmup reconfirmed on the plain <!-- stale? -->
  ...
  after a deploy.
- 2026-07-30 — `MESH_CA_PIN` in fly.toml pins the derived mesh CA <!-- stale? -->
...
- 2026-07-31 — **cp1 reprovisioned onto the new disk layout**: ~~node name <!-- stale? -->
  ...
  at `/var/mnt/media`.
- 2026-07-31 — **a plain `talosctl reset` wipes the whole disk, <!-- stale? -->
...
- 2026-07-31 — the media library is **empty** post-migration (old <!-- stale? -->
...
- 2026-07-31 — **Talos enforces the `baseline` Pod Security Standard on <!-- stale? -->
...
- 2026-07-31 — services are now `http://<name>.cp1.mesh.internal/` <!-- stale? -->
...
- 2026-07-31 — deleting an ArgoCD Application without the <!-- stale? -->
...
- 2026-07-31 — **ArgoCD polls git every ~3 min**; a push is not a <!-- stale? -->
...
- 2026-07-31 — **siwe-oidc restart semantics differ per relying <!-- stale? -->
...
- 2026-07-31 — the siwe-oidc image deploys as `:latest` + <!-- stale? -->
...
- 2026-07-31 — ghcr package `siwe-oidc` came up **public** on first <!-- stale? -->
  push (no manual visibility flip was needed).

- 2026-07-31 — **a machine waiting on device-flow approval has no apid**: <!-- stale? -->
  ...
  first boot.
- 2026-07-31 — **`install.disk: /dev/sda` in `base/worker.yaml` is a <!-- stale? -->
  ...
  *both* `disk` and `diskSelector` set with an implicit precedence rule
  deciding the winner.
- 2026-07-31 — **a worker cannot reuse the cluster layer.** <!-- stale? -->
  ...
  changes.
- 2026-07-31 — `talosctl` global flags are **not** global: `-n`/`-e` must <!-- stale? -->
  follow the subcommand (`talosctl version --insecure -n IP`, not
  `talosctl -n IP version`), and maintenance-mode reads are `get -i`,
  not `--insecure`.
- 2026-07-31 — **`~/.kube/config` is dead**: it still routes through <!-- stale? -->
...
- 2026-07-31 — **the media library is empty and declared disposable**, <!-- stale? -->
...
- 2026-07-31 — **Nothing on this cluster is replicated.** Longhorn is <!-- stale? -->
  ...
  safe".
- 2026-07-31 — **Knowing deviation from invariant 2** (decision <!-- stale? -->
...
- 2026-07-31 — **Anything that dials a `.mesh.internal` name from a pod <!-- stale? -->
...
- 2026-07-31 — `talosctl wipe disk <part>` does **not** free a user <!-- stale? -->
...
- 2026-07-31 — cp1's DHCP lease moved four times in one day <!-- stale? -->
...
- 2026-09-03 — **Beads config lives outside git and can vanish.**
  `.beads/config.yaml` + `metadata.json` are gitignored; if they go
  missing `bd` silently falls back to an empty DB named `beads` and
  reports "no open issues" — it is not empty, it is *unconfigured*.
  Symptom: `bd dolt show` says `Database: beads` instead of
  `talos_config`. Fix: restore `metadata.json` with
  `"dolt_database": "talos_config"`, or `bd bootstrap` after writing it.
  `export.auto` is now `false` (the auto `git add` of
  `.beads/issues.jsonl` fails on the gitignored dir and warned on every
  write). Session-close check: `git ls-remote origin refs/dolt/data`
  should move after `bd dolt push`.
- 2026-09-03 — **Mesh v3 pivot is live in the tracker, not in code.**
  Nothing about the running system changed; nebula is the mesh until
  Phase 4. Deferred nebula-era issues (`cjo en6 4ns 41b 6gq ap2 90a`)
