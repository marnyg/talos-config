...
# ghcr.io/marnyg/gateway:<rev>. fly/deploy.sh's shape: the build runs
...
# image@digest in deployment.yaml and let ArgoCD sync.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

attr=".#packages.x86_64-linux.gateway-image"
store=()
...
image="ghcr.io/marnyg/gateway:$tag"
