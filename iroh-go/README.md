...
| target | status | how |
|---|---|---|
| **(a) fly container, linux/amd64 (hub)** | **linux verified on aarch64** (`nix build .#iroh-go-smoke` inside a `nixos/nix` container: both smokes pass; 16.1 MB ELF, `NEEDED` = libc/libm/libdl/libpthread/libgcc_s only). **x86_64-linux verified in CI** (`.github/workflows/iroh-go.yml`, run 34694970750: both smokes + drift green; 18.1 MB glibc-dynamic ELF, same `NEEDED` set). | `nix build .#iroh-go-smoke` on an x86_64-linux builder: glibc-dynamic Go binary with iroh inside; drop into the existing `nix2container` image the same way as `config-server-bin`. Needs `pkgs.stdenv.cc` at build time only. Nothing else at runtime beyond glibc. |
